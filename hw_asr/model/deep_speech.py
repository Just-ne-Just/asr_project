...
class RNNBatchNorm(nn.Module):
    def __init__(self, input_size, hidden_size, bidirectional, rnn_type, *args, **kwargs) -> None:
...
class DeepSpeech(BaseModel):
    def __init__(self, n_feats, n_class, rnn_type='gru', rnn_hidden=1024, bidirectional=True, num_conv_layers=2, num_rnn_layers=5, **batch):
        ...
        for i in range(num_rnn_layers):
            if i == 0:
                ...
                rnn_i = nn.Sequential(
                    RNNBatchNorm(input_size=rnn_hidden, hidden_size=rnn_hidden, rnn_type=rnn_type, bidirectional=bidirectional),
                )
    ...
    def forward(self, spectrogram, **batch):
        x = self.conv_layers(spectrogram.unsqueeze(0).transpose(2, 3).transpose(0, 1))
